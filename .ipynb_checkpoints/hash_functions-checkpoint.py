...
def h_polynomial_rolling(key, N, p=53, m=2**64):
    '''
    Uses a string(key) and sums a scaled ascii representation and returns the
    hash value according to the appropriate reducer(N - typically tablesize)
    p - a prime number close to the number of characters in the input alphabet
    m - large number, since the probability of two random strings colliding is
